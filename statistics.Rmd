...
Distance: Distance is not a significant predictor in this model, with p = 0.653. It is weakly associated with pace in a negative direction - longer runs tend to have a lower (faster) pace, which is interesting and slightly unexpected.

Suffer score: Suffer score is not a significant predictor in this model, with p = 0.185. It is weakly associated with pace in a positive direction - higher suffer scores are associated with higher numbers for pace (slower runs).

Heartrate: Heartrate is a significant predictor of pace with p = 0.001. It is associated with pace in a negative direction - higher heartrates are associated with lower (faster) paces.
...
Distance: Distance is not a significant predictor in this model, with p = 0.106. It is weakly associated with pace in a negative direction - longer distances are associated with lower (faster) paces.

Suffer score: Suffer score is also not a significant predictor in this model, with p = 0.059. It is associated with pace in a positive direction - higher suffer scores are associated with higher numbers for pace (slower runs).

Heartreate: Heartrate is no longer a significant predictor of pace in this model, with p = 0.084. It is associated with pace in a negative direction - higher heartrates are associated with lower (faster) paces.
...
summary(model_sleep)
```
Distance: Distance is not a significant predictor in this model, with p = 0.652. It is weakly associated with pace in a positive direction - longer runs tend to have a higher (slower) pace.

Suffer score: Suffer score is not a significant predictor in this model, with p = 0.182. It is weakly associated with pace in a positive direction - higher suffer scores are associated with higher numbers for pace (slower runs).

Heartrate: Heartrate is a significant predictor of pace with p = 0.001. It is associated with pace in a negative direction - higher heartrates are associated with lower (faster) paces.

Temperature: Temperature is the strongest predictor in the model, with p < 0.001. As temperature increases, running pace also increases (i.e., when it is hotter, I tend to run slower).

Sleep: Hours of sleep the night before a run is not a significant predictor, with p = 0.651. It is weakly associated with pace in a negative direction - getting more sleep is associated with lower (faster) pace.
...
```
The AIC scores are very similar. The model that does not include sleep has a slightly lower score, which means it does a bit better job of explaining our data. This is the model we will visualize.
...
#### ANOVA
To find out if the mean pace differs by season, we can use an ANOVA test.
```{r}
season_model <- aov(runs$average_speed_mpm ~ runs$season)
anova(season_model)
```
The ANOVA indicates that there are no significant differences in mean running pace across seasons. This result is interesting because temperature was such a strong predictor in the linear regression models.
...
The QQ plot has several gaps in the top right area, suggesting that the population might not have a normal distribution. ANOVA requires a normally distributed population, so we can use the Kruskal-Wallis nonparametric test instead to test the same thing.
```{r}
kruskal.test(runs$average_speed_mpm ~ runs$season)
```
 There is still no evidence that running pace differs by season based on the Kruskal-Wallis test.
